...
- MyBatis中可以面向接口操作数据，要保证两个一致
  ...
  - mapper接口中**方法的方法名**和映射文件中编写SQL的标签的**id**属性保持一致
```xml
<?xml version="1.0" encoding="UTF-8" ?>  
<!DOCTYPE mapper  
...
第二种，在where id in后的	`(   ) `中使用< foreach >

第三种，用	`open = "("  close = ")"`代替原SQL语句的括号。
...
public void testbatchDeleteUser(){
    ...
    mapper.batchDeleteUser(strings);
}
```
## 动态设置表名
- 只能使用${}，因为表名不能加单引号
     ...
     - id：表示自定义映射的**唯一标识**，不能重复
      ...
      - type：查询的数据要映射的**实体类的类型**  
        ...
        - result：设置**普通字段**的映射关系  
          ...
          - property：设置映射关系中**实体类**中的**属性名**  
          - column：设置映射关系中**表**中的**字段名**
...
1. 可以通过为字段起**别名**的方式，保证和实体类中的属性名保持一致 
...
```
### 使用association处理映射关系
- association：处理**多对一**的映射关系
  - property：需要处理多对的映射关系的**属性名**
  - javaType：该属性的**类型**
...
<resultMap id="empAndDeptByStepResultMap" type="Emp">
    ...
    <association property="dept"
                 select="com.yumo.mybatis.mapper.DeptMapper.getEmpAndDeptByStepTwo"
...
<resultMap id="getDeptAndEmpByStepResultMap" type="dept">
    ...
    <collection property="emps"
                select="com.yumo.mybatis.mapper.EmpMapper.getDeptAndEmpByStepTwo"
                column="did"
                fetchType="lazy"/>
</resultMap>
<!--Dept getDeptAndEmpByStepOne(@Param("did") Integer did);-->
...
- fetchType：可以通过该属性手动控制延迟加载的效果，fetchType="lazy(延迟加载)|eager(立即加载)"
...
- 使一级缓存失效的四种情况：  
  ...
  - 同一个SqlSession两次查询期间执行了**任何**（无论成败、无论表中数据是否变化）一次增删改操作
...
- 如果出现报错：`Exception getting JDBC Driver`，可能是pom.xml中，数据库驱动配置错误
 - dependency中的驱动和mybatis-generator-maven-plugin插件中的驱动，两者的驱动版本应该相同
...
```tex
Page{count=true, pageNum=2, pageSize=4, startRow=4, endRow=8, total=10, pages=3, reasonable=false, pageSizeZero=false}[Emp(eid=6, empName=田七, age=18, sex=男, email=357, did=2), Emp(eid=10, empName=s, age=null, sex=null, email=null, did=null), Emp(eid=11, empName=a1, age=23, sex=男, email=123@qq.com, did=null), Emp(eid=12, empName=a2, age=23, sex=男, email=123@qq.com, did=null)]
```
#### 方法二使用PageInfo
...
```apl
PageInfo{
	pageNum=2, pageSize=4, size=4, startRow=5, endRow=8, total=10, pages=3, list=Page{
		count=true, pageNum=2, pageSize=4, startRow=4, endRow=8, total=10, pages=3, reasonable=false, pageSizeZero=false
	}[
        Emp(eid=6, empName=田七, age=18, sex=男, email=357, did=2), Emp(eid=10, empName=s, age=null, sex=null, email=null, did=null), Emp(eid=11, empName=a1, age=23, sex=男, email=123@qq.com, did=null), Emp(eid=12, empName=a2, age=23, sex=男, email=123@qq.com, did=null)],
